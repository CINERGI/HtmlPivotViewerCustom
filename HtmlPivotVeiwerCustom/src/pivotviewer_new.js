...
(function ($) {
    ...
    $.subscribe("/PivotViewer/ImageController/Collection/Loaded", function (event) {
        ...
        for (var i = 0; i < PivotCollection.config.views.length; i++) {
            ...
            LoadScript("src/views/" + viewName.toLowerCase() + "view.min.js");
            eval("var view = new PivotViewer.Views." + viewName + "View()");
        ...
        $(".pv-toolbarpanel-sort").on("mousedown", function (e) {
            if ($(this).attr("dirty") == 1) InitVisibleSelect("#" + $(this).attr("id"), CleanName($('.pv-filterpanel-search').val().toLowerCase()));
            $(this).attr("dirty", 0);
        });
    ...
    InitVisibleSelect = function (id, search, showFilterInvisible) { //hack to avoid .hide() in IE
        if (search == undefined) search = "";
        ...
        for (var i = 0; i < select.length; i++) {
            ...
            for (var j = 0; j < settings.visibleCategories.length; j++) {
                ...
                if ((category.IsFilterVisible || showFilterInvisible) && (index == selValue || CleanName(category.Name).toLowerCase().indexOf(search) != -1)) 
