...
var Settings = {
    showMissing: _showMissing, visibleCategories: undefined, disabledCategories: undefined };
...
(function ($) {
    ...
    PV.filterCollection = function (filterChange) {
	    ...
	    PV.filterViews();
        //RENE: Why are we not using $.publish("/PivotViewer/Views/Filtered", [{tiles: _tiles, filter: _filterList, sort: _currentSort, stringFacets: stringFacets}]);
    };
    ...
    var allVisibleFilterCategories = function () {
        var FilterVisibleCategories = [];
        for (var i = 0; i < PivotCollection.categories.length; i++) {
            if (PivotCollection.categories[i].isFilterVisible) FilterVisibleCategories.push(i);
            // if (PivotCollection.FacetCategories[i].IsSearchVisible) FilterVisibleCategories.push(i);
        }
        return FilterVisibleCategories;
    };

    var enabledFilterCategories = function (disabledCategories) {
    ...
    var enabledMetadataCategories = function (disabledCategories) {
        var MetadataCatagories = [];
        for (var i = 0; i < PivotCollection.categories.length; i++) {
            // if (PivotCollection.FacetCategories[i].IsFilterVisible) SearchVisibleCategories.push(i);
            if (PivotCollection.categories[i].isMetaDataVisible)
    ...
    var enabledSearchableLongStringCategories = function(disabledCategories) {
        var SearchVisibleCategories = [];
        for (var i = 0; i < PivotCollection.categories.length; i++) {
            // if (PivotCollection.FacetCategories[i].IsFilterVisible) SearchVisibleCategories.push(i);
            if (PivotCollection.categories[i].IsSearchVisible &&
                PivotCollection.categories[i].isLongString()) {
    ...
    $.subscribe("/PivotViewer/Settings/Changed", function (event) {
        ...
        var enanbleFirstCategory = enabledCatagories[0];
        var selCategory = PivotCollection.categories[enanbleFirstCategory];
        if (selCategory != undefined && !selCategory.uiInit) PV.initUICategory(selCategory);
        PV.selectView(0);
        ...
        LoadSem.acquire(function (release) {
            ...
            for (var i = 0; i < enabledCatagories.length; i++) {
                var category = PivotCollection.categories[enabledCatagories[i]];
    ...
    $.subscribe("/PivotViewer/ImageController/Collection/Loaded", function (event) {
        ...
        for (var i = 0; i < PivotCollection.categories.length; i++) {
            var category = PivotCollection.categories[i];
            if (category.IsFilterVisible)  html += "<option value=" + i + " search='" + PV.cleanName(category.name.toLowerCase()) + "'>" + category.name + "</option>";
        }
        ...
        $("#pv-settings-submit").click(function (e) {
            ...
            var allList = $("#pv-all-columns option")
     
            //Settings.showMissing = $("#show-missing").prop("checked");
            // something changed on page... where is the all
        ...
        $('.pv-toolbarpanel-view').click(function (e) {
            ...
            if (viewId != null) PV.selectView(parseInt(viewId));
        });
        ...
        LoadSem.acquire(function (release) {
            ...
            var enabledCatagories = enabledFilterCategories(Settings.disabledCategories);
            if (enabledCatagories.length < PivotCollection.categories.length)//settings.visibleCategories
    ...
    $('#pv-toolbarpanel-search').autocomplete({
        ...
        select: function (event, ui) {
            ...
            for (var i = 0; i < _filterList.length; i++) {
                if (_filterList[i].item.Id == selectedItem.Id) {
                    //var tile = _filterList[i - 1];
    ...
    $.subscribe("/PivotViewer/Views/Item/Selected", function (evt) {
        ...
        if (selectedItem != null) {
            ...
            for (var i = 0; i < facets.length; i++) {
                ...
                //if (!Settings.visibleCategories[category.name]) continue; // suave
                ...
                if (IsMetaDataVisible) {
                    ...
                    for (var j = 0; j < facets[i].values.length; j++) {
                        ...
                        detailDOM[i] += "<div pv-detail-item-value='" + value.value +
                            "' class='pv-infopanel-detail-item detail-item-value" + (IsFilterVisible ? " detail-item-value-filter" : "") + "'>";
